...
try:
    from ccm import (
        ...
        bootstrap,
        buscar_productos_1688_imagen,
        buscar_productos_1688_texto,
        ...
        generar_pdf_confirmacion_cotizacion,
        generar_pdf_etiqueta_proveedor,
        get_db,
        get_tarifa,
        guardar_permisos,
        hash_pwd,
    ...
    falta_ccm = modulo == "ccm" or str(_exc).startswith("No module named 'ccm'")
    if falta_ccm:
        st.error(
            "Falta el archivo **`ccm.py`** junto a `app.py`. En GitHub abra el repo "
            "`casilleros-ccm` → **Add file → Create new file**, nómbrelo exactamente "
            "`ccm.py` (un archivo, **no** una carpeta), pegue el contenido, haga commit "
            "y luego **Manage app → Reboot**. Si existe una carpeta vacía `ccm/`, bórrela."
        )
    else:
        st.error(
    ...
    raiz = sorted(p.name for p in _ROOT.iterdir() if p.suffix in {".py", ".txt", ".toml"} or p.is_dir())
    st.caption("Archivos en la raíz: " + (", ".join(raiz) if raiz else "(vacío)"))
    st.caption("ccm.py presente: " + ("sí" if (_ROOT / "ccm.py").is_file() else "no"))
    st.stop()
